...
['validate-cpf', 'CPF inválido, preencha no formato requerido: 999.999.999-99', function(v, elm) {
	return Cerebrum.Telencephalon.validaCPF(v,elm);
}],

['validate-cnpj','CNPJ Inválido, preencha no formato requerido', function(v,elm){
	return Cerebrum.Telencephalon.validaCNPJ(v,elm);
}]
...
function FIX_IWD_OnepageCheckout_shipping_payment() {
...
function FIX_IWD_Opc_shipping() {

	if (IWD.OPC.Checkout.updatePaymentBlock == true){
		console.log('FIXED: IWD_Opc 3.1.3');
		return;
	}


	console.log('FIX: IWD_Opc - Ao selecionar determinado método de entrega é feito a atualização do bloco "Payments" dessa forma deve funcionar a exibição dos valores das parcelas');

	var interval_IWD = setInterval(function(){

		if (IWD.OPC.Checkout.ajaxProgress != false){

			console.log('FIX: IWD_Opc - IWD.OPC.Checkout.pullPayments();');
...
function ItemInstallments_CheckoutTelencephalon(event, element){
	...
	var payment_method = element_name.replace("payment[cc_installments__","");
	var payment_method = payment_method.replace("]",""); 
	...
	new Ajax.Request( URL_FEE, {
		...
		onComplete: function(transport) {
			...
			if( $('onepagecheckout_orderform') ){
				...
				FIX_IWD_OnepageCheckout_installments();
...
function Telencephalon_Checkout_Type(event, element){
	...
	if( (element_method == 'cerebrum_cielo') ){

		/*if( $('cerebrum_cielo_cc_number') ){
		...
		if(element_value == 'visa_electron'){
			alert('Os bancos liberados para transações com a opção Débito VISA Electron: Banco Bradesco, Banco do Brasil, Santander, HSBC, Itaú, Mercantil, Sicredi, Banco de Brasília, Banco da Amazônia, Banco Espírito Santo e Banco do Nordeste.');
		}

		if(element_value == 'maestro'){
			alert('Os bancos liberados para transações com a opção Débito MAESTRO: Banco Santander, Banco de Brasília e Bancoob.');
		}
	...
	//console.log(form_id);
	//console.log(document.forms[form_id]);
	//console.log(element_name_parcela);
	//console.log(element_storage);
	...
	for ( var i = 0; i < GET_element_name_parcela.length ; i++ ) {
		...
		element_item_name = element_item.name;
		element_item_value = element_item.value;
		element_parentNode = element_item.parentNode.parentNode.parentNode.parentNode;
...
jQuery( document ).on( 'click', function( event ) {
	...
	ItemInstallments_CheckoutTelencephalon(event, element);
...
function Phone_Event(event) {
	...
	element.removeClassName("validate-telefone-br");
	element.removeClassName("validate-telefone-br-nove-mascarado");

	if(event.type == 'blur') {
		if(phone.length > 10) {
			element.addClassName("validate-telefone-br-nove-mascarado");
		} else {
	...
	if(phone.length > 10) {
		txtBoxFormat(element, '(99) 99999-9999', event);
	} else {
		txtBoxFormat(element, '(99) 9999-9999', event);
	}
...
var Cerebrum=Cerebrum||{};

Cerebrum.Telencephalon = {
		
		observer: {},		
		
		validaCPFouCNPJ: function(cpf,pType){				
			...
			if(pType <= 1){
			if ( ( cpf_filtrado.substring(9,11) == Cerebrum.Telencephalon.checkCPF( cpf_filtrado.substring(0,9) ) ) && ( cpf_filtrado.substring(11,12)=="") ){return true;}
			}

			if((pType == 2) || (pType == 0)){
			if (cpf_filtrado.length >= 14){
			if ( cpf_filtrado.substring(12,14) == Cerebrum.Telencephalon.checkCNPJ( cpf_filtrado.substring(0,12) ) ){ return true;}
			}
			}
        ...
        function onReadyStateChange() {
            if(self.readyState == 4 /* complete */) {
				...
				if (typeof(json.error) != "undefined"){
					...
					if ( $('cerebrum_cielo_cc_owner') ){
						$('cerebrum_cielo_cc_owner').setValue('');
						$('cerebrum_cielo_cc_number').setValue('');
						$('cerebrum_cielo_expiration').setValue('');
						$('cerebrum_cielo_expiration_yr').setValue('');
						$('cerebrum_cielo_cc_cid').setValue('');
					}
